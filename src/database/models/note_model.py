...
class NoteTable(MainBase):
    theme: Mapped[str] = mapped_column(
        type_=String(100), name="theme_note", unique=False, index=True, nullable=False
    )
    ...
    id_user: Mapped[int] = mapped_column(
        ForeignKey("UserTable.id"), type_=Integer, name="id_user", nullable=False
    )

    user: Mapped["UserTable"] = relationship(
        "UserTable", back_populates="notes", uselist=False
    )
